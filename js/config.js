...
(function (root, factory) {
  ...
  return {
    ...
    how: [
      ...
      /* THE CONTROLS live in the codex too, so they are one button away when you forget which
       * key lifts and which one lays — and so the controller has somewhere to be written down. */
      { marker: 'keys',    group: 'Controls', title: 'Keyboard & mouse', body: 'WASD or the ARROW KEYS move — push into a wall to climb it, hold SHIFT to walk back down, SPACE to jump (badly; you are meant to climb). The MOUSE looks. E or a CLICK pulses. F uses whatever you are standing at — reads a gauge, feeds a forge. Q lifts the block you are facing (and takes one back out of a forge); R lays down the one in your arms. 1–6 or the mouse WHEEL choose a pocket. C opens and closes this codex.' },
      { marker: 'gamepad', group: 'Controls', title: 'Controller', body: 'A gamepad works everywhere the keyboard does — plug it in and press any button. On the title screen the D-PAD (or left stick) picks a level and A starts it. In the warren: LEFT STICK moves (push into a wall to climb it); RIGHT STICK looks. RT (right trigger) pulses. X lifts a block (and takes one back out of a forge); Y lays one down; B uses what you are at — a gauge, a forge. LT (left trigger) walks you down a wall. LB and RB step through your six pockets. A jumps. START opens this codex.' }
    ],
