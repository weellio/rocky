...
group('EVERY ROOM HAS A WAY OUT, AND IT CALLS', () => {
  ...
  for (const c of CFG.chapters) {
    ...
    const isPuzzle = (c.walk && c.walk.length) || S.doors.length || S.gauges.length || (c.track && c.track.need) || c.seal || c.count || (c.build_target && c.build_target.length) || c.clear || c.bred || c.contain || c.hold;
    steps(S, 6);
    if (isPuzzle) {
